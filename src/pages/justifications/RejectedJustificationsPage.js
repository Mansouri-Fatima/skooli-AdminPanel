...
export default function RejectedJustificationsPage() {
...
const API_BASE_URL = "http://localhost:5001/api"
  ...
  useEffect(() => {
  const fetchRejectedJustifications = async () => {
    setLoading(true)
    setError(null)

    try {
      const response = await axios.get(`${API_BASE_URL}/attendance/justifications/rejected`)
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {justifications.map((justification) => (
                <tr key={justification.id} className="h-[80px]">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="px-4 py-1 rounded-full text-xs    bg-[#FFE0E6] text-[#FF4974] border border-[#FF4974]">
